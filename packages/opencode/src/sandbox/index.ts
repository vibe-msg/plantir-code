...
import { USER_SETTINGS_DIR, SETTINGS_DIRECTORY_NAME } from "./config"
...
export async function start_sandbox(config: SandboxConfig, nodeArgs: string[] = []) {
  if (config.command === "sandbox-exec") {
    ...
    const args = [
      ...
      "sh",
      "-c",
      [`SANDBOX=sandbox-exec`, `NODE_OPTIONS="${nodeOptions}"`, ...process.argv.map((arg) => quote([arg]))].join(" "),
    ]
  ...
  let index = 0
  const containerNameCheck = execSync(`${config.command} ps -a --format "{{.Names}}"`).toString().trim()
